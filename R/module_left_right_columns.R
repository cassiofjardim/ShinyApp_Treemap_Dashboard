    ...
    div(
      ...
      tabsetPanel(id = 'right_panels',
        ...
        tabPanel(
         ...
         tags$header(style = 'width: 100%; height: 200px;
    background: lightslategray; margin: 1em 0;border-radius: 5px; padding: 1em;',
               ...
               function(input, output, session) {
                 ...
                 output$chart_treemap <- renderHighchart({
                   league_table() %>%
                     ...
                     treemap_chart_function(slices_colors = colors,
...
#*******************************************************************************
                 output$season_performance_chart <-
                   renderHighchart({
                     df_weeks_perforance %>%
                       ...
                       spline_chart_function(add_ts_name = 'Arsenal', color = 'red')
                   })
                 ...
                 output$performance_description <- renderUI({
                   div(class = 'performance_comment',
                       ...
                       h5(glue::glue("During the 38 Weeks The Squad of {rv$country} started with
                                     {df_weeks_perforance %>% select('Arsenal') %>% pluck(1,1)} and ended
                                     {df_weeks_perforance %>% select('Arsenal') %>% pluck(1,38)}
                                     ")))
                 })
                   ...
                   output[[paste0('xg_chart_', x)]] <- renderHighchart({
                     league_table_df %>%
                       ...
                       hchart(
                         ...
                         hcaes(
                           x = Squad,
                           y = league_table_df[[!!var]],
                           color = league_table_df$colors
                         )
                       )%>% column_chart_function(y_var = panels_title[x],
                                                  color = 'red',
                                                  mean_line = mean(league_table_df[[panels_title[x]]]))
...
#***********************************************************************************
          ...
          #              ))
                 ...
                 panels_title_right <-
                   c('Gls', 'Ast', 'G_plus_A', 'xG_plus_xAG_Per_Minutes')
                 ...
                 charts_card_two <- function(x, var) {
                   ...
                   output[[paste0('standard_chart_', x)]] <- renderHighchart({
                     ...
                     )%>% column_chart_function(y_var = panels_title_right[x],
                                                color = 'red',
                                                mean_line = mean(standard_stats_df[[panels_title_right[x]]]))
...
#***********************************************************************************
          ...
          #              ))
                 ...
                 panels_title_bottom_left <-
                   c('xG_Home', 'xG_Away', 'G_xG_Home', 'G_xG_Away')
                   ...
                   output[[paste0('home_away_chart_', x)]] <- renderHighchart({
                     hchart(
                       ...
                       hcaes(
                         ...
                         y = round(home_away_stats_df[[!!var]], 2),
                         color = home_away_stats_df$colors
                       )
                     ) %>% bubble_chart_function(y_var = panels_title_bottom_left[x])
                 ...
                 1:length(panels_title_bottom_left) %>%
                   map( ~ charts_card_three(x = .x, var = panels_title_bottom_left[.x]))




                 output$progression_chart_1 <- renderHighchart({
                   hchart(
                     home_away_stats_df,
                     'bubble',
                     showInLegend = F,
                     hcaes(
                       x = Squad,
                       group = home_away_stats_df$Squad,
                       y = home_away_stats_df$xG_Home,
                       color = home_away_stats_df$colors
                     )
                   ) %>% bubble_chart_function(y_var = 'xG_Home')

                 })


                 output$chart_5_chart_1 <- renderHighchart({
                   hchart(
                     home_away_stats_df,
                     'bubble',
                     showInLegend = F,
                 ...
                 output$chart_6_chart_1 <- renderHighchart({
                   hchart(
                     home_away_stats_df,
                     'bubble',
                     showInLegend = F,
